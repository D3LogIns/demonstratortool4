...
{
    if(x<480 && y<272)
        frameBuffer16BitTft800480[(y)][(x)]=( frameBuffer16BitTft800480[(y)][(x)]
         & (0x0) | (( c)&0x00ffffff));                       
...
void Ramchange(void)
{
  ...
  for (i=0;i<272;i++)
  {
	addr_offset=(272-1-i)*480;
//	Delay(10);
	for (j=0;j<480;j++)
	{
									
	frameBuffer16BitTft800480[(i)][(j)]=((Image_data[addr_offset])&0x00ffffff);   
 ...
 void WBG() 
{
  ...
  for (j=0;j<320;j++)
  {
   for (i=0;i<34;i++)
   {
     for (k=0;k<8;k++)
     {
       PutPixel(24*i+k,j,0x00000000);
     }
     for (k=8;k<16;k++)
     {
       PutPixel(24*i+k,j,0x00FFFFFF);
     }
     for (k=16;k<24;k++)
     {
       PutPixel(24*i+k,j,0x007F7F7F);
     }
   }
  }
}
    ...
    for (j=0;j<320;j++)
    {
    	for (i=0;i<480;i++)
  		{ 
   			...
   			k=(((j*0x100)/320)<<16)+(((j*0x100)/320)<<8)+((j*0x100)/320); 
    ...
    for (j=0;j<88;j++)
    {
    	for (i=0;i<800;i++)
  		{ 
   			...
   			k=(((i*0x100)/800)<<16)+(((i*0x100)/800)<<8)+((i*0x100)/800); 
