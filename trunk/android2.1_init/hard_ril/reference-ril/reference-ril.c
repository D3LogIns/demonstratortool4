...
static void onSIMReady()
{
     ...
     * bfr = 1  // flush buffer
     */
	at_send_command("AT+CNMI=2,2", NULL);  //Figo    at_send_command("AT+CNMI=1,2,2,1,1", NULL);
}
    ...
    for (;;) {
        ...
        while  (fd < 0) {
            if (s_port > 0) {
                ...
                if ( fd >= 0 && (!memcmp( s_device_path, "/dev/ttyS", 9 ) || !memcmp( s_device_path, "/dev/s3c2410", 12 ) ) ) {//Figo
